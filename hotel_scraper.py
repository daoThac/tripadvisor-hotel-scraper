    ...
    with Stealth().use_sync(sync_playwright()) as p:
        ...
        with open('hotels_hue_reviews_jan2025.csv', 'a', encoding='utf-8', newline='') as f2:
            ...
            for idx, hotel in enumerate(hotel_list):
                ...
                # BẢO VỆ LỚP 1: Chỉ xử lý KS có URL từ Huế trong danh sách đã lọc
                if 'Thua_Thien_Hue_Province' not in hotel['hotel_url']:
                    print(f"  [BỎ QUA] URL không phải Huế: {hotel['hotel_url']}")
                    continue
                ...
                try:
                    ...
                    # BẢO VỆ LỚP 2: Kiểm tra URL thực tế sau khi tải trang
                    # TripAdvisor đôi khi redirect sang KS quảng cáo tại địa điểm khác
                    actual_url = page.url
                    if 'Thua_Thien_Hue_Province' not in actual_url:
                        print(f"  [BỎ QUA - REDIRECT] Trang đã bị chuyển hướng sang URL ngoài Huế!")
                        print(f"  URL đích: {actual_url}")
                        continue
                    ...
                    while True:  
                        ...
                        for card in review_cards:
                            try:
                                ...
                                # BẢO VỆ LỚP 3: Luôn dùng URL gốc của KS (không dùng actual_url)
                                # để đảm bảo URL trong CSV khớp với danh sách Huế
